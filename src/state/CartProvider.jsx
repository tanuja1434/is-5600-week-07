...
const cartReducer = (state, action) => {
  ...
  return {
    ...
    itemsById: {
      ...state.itemsById,
      [payload._id]: { // use _id to match keys in itemsById
        ...state.itemsById[payload._id],
        quantity: payload.quantity < 1 ? 1 : payload.quantity, // prevent < 1
      },
    },
  };
...
const CartProvider = ({ children }) => {
 ...
 const updateItemQuantity = (productId, quantity) => {
  dispatch({
    ...
    payload: { _id: productId, quantity: quantity < 1 ? 1 : quantity } // clamp here too
  });
};
